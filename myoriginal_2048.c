...
void key_function(int board[height][width], int key, int *sum_point, int *merge_number);
void push_zero_side(int board[height][width], int org_board[height][width], int key);
void cal_new_board(int board[height][width], int org_board[height][width], int key, int* sum_number, int *merge_number);
...
int main (int argc, char *argv[]) {
    ...
    int key;
    time_t pass_time, remain_time;
    while(((c = getchar()) != '.')&&(remain_time > 0)) {
        system("clear");
        merge_number = 0;
        if ((c == 'k')||(c == 'i')||(c == 'j')||(c == 'm')) {
            if (c == 'k'){
            ...
            key_function(board, key, &sum_point, &merge_number);
            ...
            if(sum_point > 600){
                printf("your point: - 600 points, your time limit: + 10 seconds\r\n");
                time_limit = time_limit + 10;
                sum_point = sum_point - 600;
            }
            else{
                printf("your point is not enough\r\n");
            }
        }
        // system("clear");
        print_boardgame(board);
        ...
        printf("Press '.' to close\r\n");
        printf("key_function:\r\n'k' : left\r\n'j' : right\r\n'i' : up\r\n'm' : down\r\n'f': use 600 points to get 10 seconds\r\n");
        printf("\r\n\r\n");
        printf("SUM POINT: %d\r\n", sum_point);
        printf("MERGE NUMBER: %d\r\n", merge_number);
        if(merge_number < 1){
            time_limit = time_limit - 2;
        }else if(merge_number >2){
            time_limit = time_limit + 1;
        }
        now_time = time(NULL);
        pass_time = now_time - start_time;
        printf("pass time: %ld\r\n", pass_time);
        remain_time = time_limit - pass_time;
        printf("remain time: %ld\r\n", remain_time);
    }
...
void key_function(int board[height][width], int key, int *sum_point, int *merge_number){
    ...
    cal_new_board(board, org_board, key, sum_point, merge_number);
        ...
        while(h != height){
            ...
            while(org_current != width){
                if(org_current == width - 1){
                    ...
                    *sum_number = *sum_number + board[h][w];
                }
                else{
                    board[h][w] = org_board[h][org_current];
                    ++org_current;
